...
function display_protein_protein_interface(example_id = "4bmg_dimer"; alpha = true, solvent_radius = 1.4, show_wireframe = false)
    ...
    interface_surface = DelaunayInterfaces.InterfaceSurface(points, color_labels, radii .+ solvent_radius, alpha)
    display(DelaunayInterfaces.get_interface_figure(interface_surface; show_wireframe = show_wireframe))
end

function display_protein_space_filling_diagram(example_id = "4bmg_dimer")
    points, color_labels, radii = get_example_data(example_id)
    display(DelaunayInterfaces.get_point_cloud_figure(points, color_labels, radii; point_cloud_colormap = :magma))
end
...
function display_protein_protein_interface_and_space_filling_diagram(example_id = "4bmg_dimer"; alpha = true, solvent_radius = 1.4, show_wireframe = false)
    points, color_labels, radii = get_example_data(example_id)
    interface_surface = DelaunayInterfaces.InterfaceSurface(points, color_labels, radii .+ solvent_radius, alpha)
    display(DelaunayInterfaces.get_interface_and_point_cloud_figure(interface_surface, points, color_labels, radii; show_wireframe = show_wireframe, point_cloud_colormap = :magma))
end
...
function display_protein_protein_interface_filtration(example_id = "4bmg_dimer"; alpha = true, solvent_radius = 1.4, show_wireframe = false)
    points, color_labels, radii = get_example_data(example_id)
    interface_surface = DelaunayInterfaces.InterfaceSurface(points, color_labels, radii .+ solvent_radius, alpha)
    display(DelaunayInterfaces.get_interface_filtration_figure(interface_surface; show_wireframe = show_wireframe))
end
